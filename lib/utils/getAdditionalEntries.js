...
function getAdditionalEntries({ devServer, options }) {
  ...
  if (devServer) {
    const { sockHost, sockPath, sockPort, host, path, port, https, http2, client } = devServer;
  ...
  if (options.client && options.client.webSocketURL) {
    options.client.webSocketURL.hostname && (resourceQuery.sockHost = options.client.webSocketURL.hostname);
    options.client.webSocketURL.pathname && (resourceQuery.sockPath = options.client.webSocketURL.pathname);
    options.client.webSocketURL.port && (resourceQuery.sockPort = options.client.webSocketURL.port);
  }
